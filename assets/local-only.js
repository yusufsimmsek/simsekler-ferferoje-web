(() => {
  ...
  function customizeHeaderButtons() {
    // 1. Remove the wrapper inside the hamburger menu so it doesn't duplicate
    const tabletWrappers = document.querySelectorAll('.search-shop-con.tablet-button-wrapper');
    tabletWrappers.forEach(w => w.remove());

    // 2. Inject CSS to force the desktop wrapper to show on mobile
    const style = document.createElement('style');
    style.innerHTML = `
      @media (max-width: 991px) {
        /* Force buttons wrapper to show next to hamburger menu */
        .search-shop-con {
          display: flex !important;
          gap: 6px !important;
          margin-right: 15px !important;
        }
        /* Hide the text inside buttons to fit on small mobile screens */
        .header-btn-text {
          display: none !important;
        }
        /* Make buttons smaller on mobile */
        .search-shop-con .primary-button {
    ...
    wrappers.forEach(wrapper => {
      ...
      wrapper.innerHTML = `
        <a href="https://wa.me/905323595349" target="_blank" class="primary-button w-inline-block" style="background-color:#25D366; border-color:#25D366; color:white; padding: 10px 15px; display:flex; align-items:center; gap:8px;">
            ...
            <div class="header-btn-text" style="font-size: 14px; font-weight: 600;">Bize Yazın</div>
        </a>
        <a href="tel:+905323595349" class="primary-button w-inline-block" style="padding: 10px 15px; display:flex; align-items:center; gap:8px;">
            ...
            <div class="header-btn-text" style="font-size: 14px; font-weight: 600;">Arayın</div>
        </a>
      `;
    });
  }
